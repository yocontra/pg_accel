    ...
    do {                                                                       \
        if ((a) != (b)) {                                                      \
            fprintf(stderr, "  FAIL %s:%d: %s (%zu) != %s (%zu)\n",           \
                    __FILE__, __LINE__, #a, (size_t)(a), #b, (size_t)(b));     \
            g_tests_failed++;                                                  \
            return;                                                            \
        }                                                                      \
    } while (0)
...
static bool has_pair(const uint32_t* pairs, size_t count,
...
TEST(empty_inputs)
{
    ...
    /* Both empty */
    s = pgaccel_spatial_intersects(
        nullptr, 0, nullptr, 0,
        r.dt.data(), &r.dt_count,
        r.df.data(), &r.df_count,
...
TEST(points_vs_polygon)
{
    ...
    assert(has_pair(r.dt.data(), r.dt_count, 3, 0) ||
           has_pair(r.unc.data(), r.unc_count, 3, 0));
}
