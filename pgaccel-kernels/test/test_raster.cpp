...
static void test_map_algebra_simple() {
    ...
    pgaccel_status st = pgaccel_map_algebra(
        bands, N, PGACCEL_PT_FLOAT32, &expr, output.data(), nodata.data());
    ASSERT_EQ(st, PGACCEL_OK, "map_algebra simple status");

    for (size_t i = 0; i < N; i++) {
        float expected = static_cast<float>(i) * 2.0f + 1.0f;
        ASSERT_NEAR(output[i], expected, 0.01,
                    "map_algebra simple pixel value");
    }
...
static void test_map_algebra_two_band() {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        ASSERT_NEAR(output[i], expected, 0.5,
                    "map_algebra two_band pixel value");
    }
...
static void test_map_algebra_int32() {
    ...
    pgaccel_status st = pgaccel_map_algebra(
        bands, N, PGACCEL_PT_INT32, &expr, output.data(), nodata.data());
...
static void test_empty_input() {
    ...
    st = pgaccel_map_algebra(
        dummy_bands, 0, PGACCEL_PT_FLOAT32, &expr,
        &dummy_out, &dummy_nd);
    ASSERT_EQ(st, PGACCEL_OK, "map_algebra empty");

    // Clip with 0 dimensions
    float ring[] = {0, 0, 1, 0, 1, 1, 0, 1};
    st = pgaccel_raster_clip(
        &dummy_in, 0, 0, 0, 0, 1, 1,
        PGACCEL_PT_FLOAT32, ring, 4,
        &dummy_out, &dummy_nd);
    ASSERT_EQ(st, PGACCEL_OK, "clip empty");

    // Reclass with 0 pixels
    pgaccel_reclass_rule rule = {0, 10, 99};
    st = pgaccel_raster_reclass(
        &dummy_in, 0, PGACCEL_PT_FLOAT32,
        &rule, 1, PGACCEL_PT_FLOAT32, &dummy_out);
...
static void test_raster_clip() {
    ...
    pgaccel_status st = pgaccel_raster_clip(
        input.data(), W, H,
        0.0, 0.0, 1.0, 1.0,
        PGACCEL_PT_FLOAT32,
        ring, 4,
    ...
    ASSERT_NEAR(output[center_idx], input[center_idx], 0.01,
                "center pixel value preserved");
    ...
    if (inside_count < 80 || inside_count > 180) {
        fprintf(stderr,
                "FAIL: clip inside count %d not in expected range [80, 180]\n",
                inside_count);
        g_tests_failed++;
        return;
    }
...
static void test_raster_reclass() {
    ...
    pgaccel_status st = pgaccel_raster_reclass(
        input.data(), N, PGACCEL_PT_FLOAT32,
        rules, 3, PGACCEL_PT_FLOAT32,
        output.data());
...
static void test_raster_reclass_mixed_types() {
    ...
    pgaccel_status st = pgaccel_raster_reclass(
        input.data(), N, PGACCEL_PT_INT32,
        rules, 2, PGACCEL_PT_FLOAT32,
        output.data());
