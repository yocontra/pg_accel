...
int main() {
  ...
  if (pid == 0) {
    ...
    // fp64 dispatch in warmed-fork mode — soft-fp64 on Metal must run
    // after pre-fork SYCL warmup. Any UNSUPPORTED/error = regression.
    ...
    if (st != PGACCEL_OK) {
      fprintf(stderr, "Child reduce_sum_f64 failed: %d (soft-fp64 post-warmup broken?)\n", st);
      _exit(5);
    }
    ...
    printf("Child: reduce_sum_f64(%zu doubles) = %f OK (soft-fp64 warmed-fork)\n", N, dsum);
    delete[] ddata;
