    ...
    for (size_t i = 1; i < n; ++i) {
        if (data[i] < data[i - 1]) {
            fprintf(stderr, "[%s] FAIL: not sorted at i=%zu (%lld < %lld)\n",
                    tag, i, (long long)data[i], (long long)data[i - 1]);
            ++g_fail;
            return;
        }
    }
...
static void check_kv_sorted_i32(const int32_t* keys, const uint32_t* indices,
                                 const int32_t* original, size_t n,
                                 const char* tag) {
    for (size_t i = 1; i < n; ++i) {
        if (keys[i] < keys[i - 1]) {
...
int main(int argc, char** argv) {
    ...
    // --- int64 ---------------------------------------------------------
    {
        ...
        for (size_t i = 0; i < n; ++i) data[i] = dist64(rng);
        std::vector<int64_t> expected(data);
        std::sort(expected.begin(), expected.end());

        std::vector<int64_t> warm(4096);
        for (size_t i = 0; i < warm.size(); ++i) warm[i] = dist64(rng);
    ...
    // --- int32 key-value -----------------------------------------------
    {
        ...
        printf("int32  kv-sort status=%d time=%.2f ms  (%.1f M/s)\n",
               (int)st, ms, n / (ms / 1000.0) / 1e6);
        check_kv_sorted_i32(keys.data(), indices.data(),
                            original.data(), n, "int32-kv");
    }
