...
int main() {
    ...
    for (size_t i = 0; i < M; ++i) ramp[i] = static_cast<float>(i + 1);
    ...
    for (size_t i = 0; i < MB; ++i) ones[i] = 1.0f;
    msum = mmin = mmax = 0.0f;
    mcount = 0;
    ...
    for (size_t i = 0; i < I; ++i) ints[i] = static_cast<int64_t>(i);
    ...
    printf("multi_i64 N=%zu: status=%d sum=%lld min=%lld max=%lld count=%lld "
           ...
           I, st,
           static_cast<long long>(isum), static_cast<long long>(imin),
