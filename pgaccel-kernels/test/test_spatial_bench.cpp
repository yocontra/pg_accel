...
static std::vector<float> make_star_polygon(size_t vc, float r_outer, float r_inner) {
...
static void compute_bbox(const float* ring, size_t vc, float* bxmin, float* bymin, float* bxmax,
                         float* bymax) {
  *bxmin = *bymin = 1e30f;
  *bxmax = *bymax = -1e30f;
...
static int run_size(size_t vc, size_t npts) {
  ...
  // Warm-up
  {
    ...
    for (size_t i = 0; i < wpts.size(); ++i)
      wpts[i] = dist(rng);
    pgaccel_point_in_polygon_bulk(wpts.data(), 1024, bbox, ring.data(), vc, nullptr, 0,
                                  wresults.data());
  }
...
int main(int argc, char** argv) {
  ...
  size_t npts = 100'000;
  if (argc > 1)
    npts = static_cast<size_t>(std::atoll(argv[1]));
