...
static void bench_map_algebra(size_t tile, size_t rows) {
  ...
  code[0].op = PGACCEL_OP_LOAD_BAND;
  code[0].arg.band_index = 0;
  code[1].op = PGACCEL_OP_LOAD_CONST;
  code[1].arg.constant = 2.0;
  code[2].op = PGACCEL_OP_MUL;
  code[3].op = PGACCEL_OP_LOAD_CONST;
  code[3].arg.constant = 1.0;
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    pgaccel_map_algebra(bands, pc, PGACCEL_PT_FLOAT32, &expr, out.data(), mask.data());
  }
  ...
  printf("map_algebra tile=%zux%zu rows=%zu  %.2f ms  "
         "(%.1f us/row, %.1f M px/s)\n",
         tile, tile, rows, ms, ms * 1000.0 / rows, total_px / (ms / 1000.0) / 1e6);
}
...
static void bench_raster_clip(size_t tile, size_t rows) {
  ...
  // Warm-up
  for (int i = 0; i < 4; ++i) {
    pgaccel_raster_clip(pixels.data(), tile, tile, 0.0, 0.0, 1.0, 1.0, PGACCEL_PT_FLOAT32, ring, 4,
                        out.data(), mask.data());
  }
...
int main(int argc, char** argv) {
  size_t rows = 100'000;
  if (argc > 1)
    rows = static_cast<size_t>(std::atoll(argv[1]));
