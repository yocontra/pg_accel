...
/// Perform grouped COUNT(*) over an already-sorted USM-accessible int64 key
/// buffer. Intended for resident caches that can pay sorting cost once at
/// load time and keep query-time work to a compact-count GPU pass.
pgaccel_agg_state* pgaccel_hash_count_i64_sorted_device_execute(int64_t* sorted_group_keys,
                                                                size_t row_count);

/// Perform grouped COUNT(*) over an existing USM-accessible int64 key buffer
/// using a row-parallel device hash table instead of sorting. Intended for
/// fused pipelines with low/medium group cardinality where sort/reduce is
/// slower than atomic count accumulation.
pgaccel_agg_state* pgaccel_hash_count_i64_device_hash_execute(int64_t* group_keys,
                                                              size_t row_count);

/// Same as pgaccel_hash_count_i64_device_hash_execute, but with a conservative
/// upper bound for distinct groups. Fused domain-specific pipelines can use
/// this to avoid clearing/compacting a hash table sized for every input row.
/// Passing 0 or a value above row_count is equivalent to row_count.
