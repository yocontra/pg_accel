...
static inline val_read read_value(const void* col_data, const uint8_t* null_mask, size_t row,
                                  int val_type) {
  val_read r = {0.0, true};
...
static inline bool keys_equal_raw(const void* keys, size_t row_a, size_t row_b, int key_type) {
  switch (key_type) {
    case 0: {
...
static pgaccel_agg_state* agg_cpu(const void* group_keys, const uint8_t* group_null_mask,
  ...
  for (size_t r = 0; r < row_count; r++) {
    ...
    for (size_t a = 0; a < num_aggs; a++) {
      ...
      if (vr.is_null)
        continue;  // Skip NULLs for SUM/MIN/MAX/COUNT(col)
...
pgaccel_agg_state* pgaccel_hash_agg_execute(const void* group_keys, const uint8_t* group_null_mask,
  ...
  return agg_cpu(group_keys, group_null_mask, row_count, key_type, value_cols, value_nulls,
                 value_types, agg_cols, num_aggs);
}
