...
static inline bool pg_lt_f64(double a, double b) {
  if (a != a)
    return false;  // NaN not less than anything
  if (b != b)
    return true;  // everything less than NaN
  return a < b;
}
...
static inline col_read_result read_col_f64(const pgaccel_batch* batch, size_t col, size_t row) {
  col_read_result r = {0.0, true};

  if (col >= batch->num_cols || batch->col_data[col] == nullptr)
    return r;
  ...
  switch (batch->col_types[col]) {
    case PGACCEL_VAL_INT32:
      r.value = static_cast<double>(static_cast<const int32_t*>(batch->col_data[col])[row]);
      break;
    case PGACCEL_VAL_INT64:
      r.value = static_cast<double>(static_cast<const int64_t*>(batch->col_data[col])[row]);
      break;
    case PGACCEL_VAL_FLOAT32:
      r.value = static_cast<double>(static_cast<const float*>(batch->col_data[col])[row]);
      break;
...
extern "C" pgaccel_status pgaccel_expr_template_cmp_const(const pgaccel_batch* batch,
  ...
  for (size_t row = 0; row < batch->num_rows; row++) {
    ...
    if (cr.is_null) {
      ...
      results[row] = cmp(cr.value, const_val) ? PGACCEL_EXPR_TRUE : PGACCEL_EXPR_FALSE;
    }
  }
  return PGACCEL_OK;
}
...
extern "C" pgaccel_status pgaccel_expr_template_between(const pgaccel_batch* batch,
  ...
  for (size_t row = 0; row < batch->num_rows; row++) {
    ...
    if (cr.is_null) {
      ...
      results[row] = (pg_ge_f64(cr.value, lo) && pg_le_f64(cr.value, hi)) ? PGACCEL_EXPR_TRUE
                                                                          : PGACCEL_EXPR_FALSE;
    }
  }
  return PGACCEL_OK;
...
extern "C" pgaccel_status pgaccel_expr_template_is_null(const pgaccel_batch* batch,
  ...
  for (size_t row = 0; row < batch->num_rows; row++) {
    ...
    if (col_idx >= batch->num_cols || batch->col_data[col_idx] == nullptr) {
      row_is_null = true;
    } else if (batch->col_nulls != nullptr && batch->col_nulls[col_idx] != nullptr &&
