...
template <typename T>
static pgaccel_hash_table* build_cpu(const T* keys, const uint8_t* null_mask,
...
template <typename T>
static pgaccel_hash_table* build_sort_merge(const T* keys, const uint8_t* null_mask,
                                            const uint32_t* indices, size_t count) {
  ...
  uint32_t* sorted_indices = static_cast<uint32_t*>(malloc(non_null * sizeof(uint32_t)));
  ...
  if constexpr (std::is_same_v<T, int32_t>) {
    ...
    st = pgaccel_sort_kv_f64(reinterpret_cast<double*>(sorted_keys), sorted_indices, non_null);
  }
...
static pgaccel_status probe_sort_merge_sycl(const pgaccel_hash_table* ht, const T* outer_keys,
  ...
  if (g_unified_memory) {
    d_counts = static_cast<uint32_t*>(sycl::malloc_shared<uint32_t>(outer_count, *q));
  } else {
  ...
  try {
    ...
    if (g_unified_memory) {
      std::memcpy(h_counts.data(), d_counts, outer_count * sizeof(uint32_t));
    } else {
...
template <typename T>
static pgaccel_status probe_sort_merge(const pgaccel_hash_table* ht, const T* outer_keys,
                                       const uint8_t* outer_null_mask, size_t outer_count,
                                       uint32_t* match_pairs, size_t max_matches,
                                       size_t* match_count) {
  return probe_sort_merge_sycl(ht, outer_keys, outer_null_mask, outer_count, match_pairs,
...
template <typename T>
static pgaccel_status probe_cpu(const pgaccel_hash_table* ht, const T* outer_keys,
                                const uint8_t* outer_null_mask, size_t outer_count,
  ...
  switch (ht->key_type) {
    case PGACCEL_KEY_INT32:
      return is_sort_merge
                 ? probe_sort_merge<int32_t>(ht, static_cast<const int32_t*>(outer_keys),
                                             outer_null_mask, outer_count, match_pairs, max_matches,
                                             match_count)
                 : probe_cpu<int32_t>(ht, static_cast<const int32_t*>(outer_keys), outer_null_mask,
                                      outer_count, match_pairs, max_matches, match_count);
    case PGACCEL_KEY_INT64:
      return is_sort_merge
                 ? probe_sort_merge<int64_t>(ht, static_cast<const int64_t*>(outer_keys),
                                             outer_null_mask, outer_count, match_pairs, max_matches,
                                             match_count)
                 : probe_cpu<int64_t>(ht, static_cast<const int64_t*>(outer_keys), outer_null_mask,
                                      outer_count, match_pairs, max_matches, match_count);
    case PGACCEL_KEY_FLOAT64:
      return is_sort_merge
                 ? probe_sort_merge<double>(ht, static_cast<const double*>(outer_keys),
                                            outer_null_mask, outer_count, match_pairs, max_matches,
                                            match_count)
                 : probe_cpu<double>(ht, static_cast<const double*>(outer_keys), outer_null_mask,
                                     outer_count, match_pairs, max_matches, match_count);
    default:
      return PGACCEL_UNSUPPORTED;
  }
