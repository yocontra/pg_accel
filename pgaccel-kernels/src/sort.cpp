    ...
    uint32_t* keys, uint32_t* indices, size_t count)
{
    ...
    try {
        ...
        auto alloc_u32 = [&](size_t n) {
            return g_unified_memory
                ? sycl::malloc_shared<uint32_t>(n, *q)
        ...
        uint32_t* d_group_hist =
            sycl::malloc_shared<uint32_t>(ngroups * RADIX_BINS, *q);

        if (!buf_keys_a || !buf_keys_b || !buf_idx_a || !buf_idx_b ||
            !d_group_hist) {
            if (buf_keys_a) sycl::free(buf_keys_a, *q);
            if (buf_keys_b) sycl::free(buf_keys_b, *q);
            if (buf_idx_a)  sycl::free(buf_idx_a, *q);
            if (buf_idx_b)  sycl::free(buf_idx_b, *q);
            if (d_group_hist) sycl::free(d_group_hist, *q);
            return PGACCEL_OOM;
        }
        ...
        if (g_unified_memory) {
            ...
            if (padded > count) {
                q->fill(buf_keys_a + count,
                        std::numeric_limits<uint32_t>::max(),
                        padded - count);
                q->fill(buf_idx_a + count,
                        std::numeric_limits<uint32_t>::max(),
                        padded - count);
            }
            q->wait();
        ...
        for (int pass = 0; pass < 4; ++pass) {
...

            {
                auto nd = sycl::nd_range<1>(
                    sycl::range<1>(ngroups * RADIX_GROUP_SIZE),
                ...
                q->submit([&](sycl::handler& h) {
                    ...
                    sycl::local_accessor<uint32_t, 1> lhist(
                        sycl::range<1>(RADIX_BINS), h);
            ...
            for (size_t b = 0; b < RADIX_BINS; ++b) bin_total[b] = 0;
            ...
            for (size_t b = 0; b < RADIX_BINS; ++b) running[b] = bin_base[b];
            for (size_t g = 0; g < ngroups; ++g) {
                for (size_t b = 0; b < RADIX_BINS; ++b) {
            ...
            std::memcpy(d_group_hist, scan_buf.data(),
                        ngroups * RADIX_BINS * sizeof(uint32_t));
            ...
            // digit as element lid).
            {
                ...
                q->submit([&](sycl::handler& h) {
                    ...
                    sycl::local_accessor<uint32_t, 1> ldigits(
                        sycl::range<1>(RADIX_TILE), h);
                    ...
                    h.parallel_for(nd, [=](sycl::nd_item<1> it) {
                        ...
                        if (my_digit != 0xFFFFFFFFu) {
                            ...
                            for (size_t i = 0; i < lid; ++i) {
                                if (ldigits[i] == my_digit) rank++;
                            }
                            const uint32_t base =
                                group_base_ptr[gid * RADIX_BINS + my_digit];
...
static pgaccel_status sycl_radix_sort_kv_u64(
    uint64_t* keys, uint32_t* indices, size_t count)
{
    ...
    try {
        auto alloc_u64 = [&](size_t n) {
            return g_unified_memory
                ? sycl::malloc_shared<uint64_t>(n, *q)
                ...
                q->submit([&](sycl::handler& h) {
                    ...
                    h.parallel_for(nd, [=](sycl::nd_item<1> it) {
                        ...
                        if (idx < padded_size) {
                            ...
                            const uint32_t d =
                                static_cast<uint32_t>((k >> shift_u) & 0xFFULL);
                            sycl::atomic_ref<uint32_t,
                                sycl::memory_order::relaxed,
                                sycl::memory_scope::work_group,
                                sycl::access::address_space::local_space>
                ...
                q->submit([&](sycl::handler& h) {
                    ...
                    h.parallel_for(nd, [=](sycl::nd_item<1> it) {
                        ...
                        if (in_range) {
                            my_key   = src_keys_ptr[tile_start + lid];
                            my_idx   = src_idx_ptr[tile_start + lid];
                            my_digit = static_cast<uint32_t>(
                                (my_key >> shift_u) & 0xFFULL);
                        } else {
        ...
        fprintf(stderr,
                "pgaccel: SYCL radix sort u64 failed: %s — falling back\n",
                e.what());
        return PGACCEL_UNSUPPORTED;
    } catch (const std::exception& e) {
        fprintf(stderr,
                "pgaccel: radix sort u64 failed: %s — falling back\n",
                e.what());
        return PGACCEL_UNSUPPORTED;
    }
}
...
static pgaccel_status sycl_radix_sort_kv_i64(
    int64_t* keys, uint32_t* indices, size_t count)
{
...
[[maybe_unused]] static pgaccel_status sycl_radix_sort_u64(
    ...
    if constexpr (is_radix_sortable<T>::value) {
        if (count >= RADIX_SORT_THRESHOLD) {
            ...
            if constexpr (std::is_same_v<T, int32_t>) {
                ...
                st = sycl_radix_sort_kv_u32(
                    reinterpret_cast<uint32_t*>(data), indices.data(), count);
                ...
                for (size_t i = 0; i < count; ++i)
                    indices[i] = static_cast<uint32_t>(i);
                st = sycl_radix_sort_kv_u64(
                    reinterpret_cast<uint64_t*>(data), indices.data(), count);
            }
    ...
    if constexpr (is_radix_sortable<K>::value) {
        if (count >= RADIX_SORT_THRESHOLD) {
            ...
            if constexpr (std::is_same_v<K, int32_t>) {
                ...
                st = sycl_radix_sort_kv_u32(
                    reinterpret_cast<uint32_t*>(keys), indices, count);
                ...
                st = sycl_radix_sort_kv_u64(
                    reinterpret_cast<uint64_t*>(keys), indices, count);
            }
            if (st == PGACCEL_OK) { pgaccel_record_gpu_exec(); return st; }
        }
    }
