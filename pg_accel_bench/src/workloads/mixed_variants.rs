...
const MIXED_SPATIAL_HEAVY_ROW_SCALES: &[usize] = &[10_000, 100_000, 1_000_000];
...
impl Workload for MixedVariant {
    ...
    fn row_scales(&self) -> &'static [usize] {
        match self.name {
            "mixed_megapoly_agg" | "mixed_spatial_sort" => MIXED_SPATIAL_HEAVY_ROW_SCALES,
            _ => crate::config::ROW_SCALES,
        }
    }
...
pub const MIXED_MEGAPOLY_AGG: MixedVariant = MixedVariant {
    ...
    query: "SELECT count(*), round(sum(val)::numeric, 6) AS total_val FROM bench_mixed_mega \
...
pub const MIXED_SPATIAL_SORT: MixedVariant = MixedVariant {
    ...
    query: "SELECT id, val FROM bench_mixed_spsort \
            WHERE ST_Intersects(geom, \
              ST_Buffer(ST_SetSRID(ST_MakePoint(-73.985, 40.748), 4326), 0.15, 125)) \
