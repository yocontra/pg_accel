...
if ! command -v nvidia-smi >/dev/null 2>&1; then
    echo "error: cuda-stress requires nvidia-smi and NVIDIA hardware" >&2
    exit 1
fi

ts="$(date +%Y%m%d-%H%M%S)"
artifact_dir="${CUDA_STRESS_ARTIFACT_DIR:-benchmarks/artifacts/cuda-stress-${ts}}"
...
iterations="${CUDA_STRESS_ITERATIONS:-20}"
warmup="${CUDA_STRESS_WARMUP:-5}"
export ACPP_TARGETS="${CUDA_STRESS_ACPP_TARGETS:-cuda}"
export GPU_TEST_TIMEOUT_S="${CUDA_STRESS_GPU_TEST_TIMEOUT_S:-300}"
...
assert_clean_logs() {
    ...
    if grep -Eiq "PGACCEL PANIC|PANIC|segmentation fault|resource leak|leaked resource|kernel failure|CUDA error|cudaError" "$combined"; then
...

{
    ...
    nvidia-smi
    cargo --version
    if [ -x ./.pgaccel/acpp/current/bin/acpp ]; then
        ./.pgaccel/acpp/current/bin/acpp --acpp-version
    elif command -v acpp >/dev/null 2>&1; then
        acpp --acpp-version
    fi
    ./pgaccel-kernels/build/test_device
} > "${artifact_dir}/metadata.txt" 2>&1

if ! grep -Eiq "Backend:[[:space:]]+cuda" "${artifact_dir}/metadata.txt"; then
    echo "error: cuda-stress did not detect an AdaptiveCpp CUDA device" >&2
    exit 1
fi
