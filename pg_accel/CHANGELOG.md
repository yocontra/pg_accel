...
- **srf**: Un-ignore passthrough test; add agg DISTINCT/ORDER/FILTER gate
- **encoder**: Emit Record SRF rows via virtual slot; build polygon[] holes via PG construct_array
- **preagg**: Nest pg_test mod inside cfg(feature=pg_test) outer mod
...
- **srf**: Add SrfTargetListPrivData variant with sentinel-prefixed round-trip
- **srf**: Wire SrfTargetList Custom Scan executor + vtables
- **srf**: Wire planner hook to inject GpuAccelSrfTargetList CustomPath
...
- **srf**: Assert GpuAccelSrfTargetList Custom Scan + expansion + passthrough
- **srf**: Mark passthrough_cols #[ignore] with anti-cheat-allow rationale
