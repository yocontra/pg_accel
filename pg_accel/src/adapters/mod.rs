...
/// - **PostGIS spatial** (`GpuSpatial`): `true` for registered fp64 recheck
///   predicates such as `st_intersects`; partial fp32-only PostGIS functions
///   stay unregistered until planner-time geometry subtype gates exist.
...
pub fn uses_fp64(strategy: AccelStrategy, name: &str) -> bool {
    match strategy {
        AccelStrategy::GpuSpatial => matches!(name, "st_intersects"),
...
mod uses_fp64_tests {
    ...
    #[test]
    fn registered_spatial_recheck_uses_fp64() {
        assert!(uses_fp64(AccelStrategy::GpuSpatial, "st_intersects"));
        assert!(!uses_fp64(AccelStrategy::GpuSpatial, "st_distance"));
    }
