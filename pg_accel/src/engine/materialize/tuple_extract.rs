...
impl AttExtractInfo {
    ...
    pub unsafe fn new(tupdesc: pg_sys::TupleDesc, attno: i32) -> Self {
        ...
        for i in 0..=target_idx {
            // SAFETY: i <= target_idx < natts, so this attribute exists.
            let attr = unsafe { &*crate::engine::pg_compat::tuple_desc_attr(tupdesc, i) };
        ...
        let target_attr =
            unsafe { &*crate::engine::pg_compat::tuple_desc_attr(tupdesc, target_idx) };
...
/// Return whether this attribute is NULL in a heap tuple header.
///
/// Unlike [`try_fast_read_heap_pub`], this distinguishes real SQL NULL from
/// "cannot use the fast offset path". Callers that build columnar batches
/// directly from `heap_getnext` can use this first, then conservatively fall
/// back to a slot/deformation path when the fast read returns `None`.
///
/// # Safety
///
/// `ht_data` must be a valid `HeapTupleHeader`. `info` must match the schema.
#[inline(always)]
pub unsafe fn heap_attr_is_null_pub(
    ht_data: pg_sys::HeapTupleHeader,
    info: &AttExtractInfo,
) -> bool {
    if ht_data.is_null() {
