...
#[must_use]
pub const fn flip_cmp_opcode(opcode: u16) -> u16 {
    match opcode {
        opcode::LT => opcode::GT,
        opcode::LE => opcode::GE,
        opcode::GT => opcode::LT,
...
mod tests {
    ...
    #[test]
    fn builder_remaps_sparse_load_cols_to_dense_indices() {
        let mut b = ExprProgramBuilder::new(8);
        b.emit_load_col(7);
        b.emit_load_col(1);
        b.emit_binop(opcode::GT);
        ...
        assert_eq!(prog.referenced_cols, vec![1, 7]);
        assert_eq!(prog.num_cols, 2);
        assert_eq!(prog.instructions[0].arg, 1);
        assert_eq!(prog.instructions[1].arg, 0);
    }

    #[test]
    fn builder_dedupes_repeated_sparse_load_cols() {
        let mut b = ExprProgramBuilder::new(8);
        b.emit_load_col(7);
        b.emit_load_col(7);
        b.emit_load_col(1);
        ...
        assert_eq!(prog.num_cols, 2);
        assert_eq!(prog.instructions[0].arg, 1);
        assert_eq!(prog.instructions[1].arg, 1);
        assert_eq!(prog.instructions[2].arg, 0);
    }
    ...
    #[test]
    fn cmp_opcode_flip_mapping() {
        assert_eq!(flip_cmp_opcode(opcode::LT), opcode::GT);
        assert_eq!(flip_cmp_opcode(opcode::LE), opcode::GE);
        assert_eq!(flip_cmp_opcode(opcode::GT), opcode::LT);
        assert_eq!(flip_cmp_opcode(opcode::GE), opcode::LE);
        assert_eq!(flip_cmp_opcode(opcode::EQ), opcode::EQ);
        assert_eq!(flip_cmp_opcode(opcode::NE), opcode::NE);
    }
