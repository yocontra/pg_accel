...
// The predicate-wide kernel lane is currently disabled. It was previously
// "disabled" by setting its min-rows threshold to `usize::MAX` — the hidden
// pattern flagged in TODO-REVIEW.md P1 ("GPU kernel branch disabled via
// usize::MAX threshold", anti-cheat rule #9). This flag makes the disablement
// explicit; re-enabling requires fixing the kernel and restoring a real
// hardware-derived threshold in `DeviceLimits`. Twin flag in
// `engine/olap_cache.rs` (`RESIDENT_DENSE_GROUP_PREDICATE_WIDE_ENABLED`) must
// stay in lockstep so scratch sizing matches dispatch.
