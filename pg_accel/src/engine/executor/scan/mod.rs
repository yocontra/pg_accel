...
pub struct ScanExecState {
    ...
    /// Optional GpuExpr input batch built directly during a heap scan.
    ///
    /// Direct `GpuExpr` scans still keep `tuple_buffer` for final PostgreSQL
    /// slot emission, but this owner lets expression kernels consume typed
    /// columns populated before the heap tuple is converted to MinimalTuple.
    /// If direct extraction cannot prove a row/column safe, dispatch falls
    /// back to the existing MinimalTuple bulk extractor for the whole batch.
    expr_batch_owner: Option<ColumnarBatchOwner>,
    ...
    /// Relation-shaped slot used only by the direct table scan producer.
    ///
    /// PostgreSQL's Custom Scan result slot can have a projected TupleDesc
    /// (for example `count(*)` does not need every base-table attribute).
    /// `table_scan_getnextslot` needs a slot matching the scanned relation,
    /// so direct GpuExpr scans keep their own buffer heap tuple slot.
    direct_scan_slot: *mut pg_sys::TupleTableSlot,

    /// Relation-shaped MinimalTuple slot used by the fallback extractor.
    ///
    /// Direct GpuExpr normally consumes `expr_batch_owner`, which is populated
    /// while the heap tuple is still live. If a batch cannot use that direct
    /// owner, `extract_f64` can still deform buffered MinimalTuples through
    /// this slot without relying on the Custom Scan result slot shape.
    direct_minimal_slot: *mut pg_sys::TupleTableSlot,
...
impl ScanExecState {
    ...
    pub fn new(
        ...
        Self {
            ...
            expr_batch_owner: None,
            scan_desc: std::ptr::null_mut(),
            direct_scan_slot: std::ptr::null_mut(),
            direct_minimal_slot: std::ptr::null_mut(),
    ...
    pub unsafe fn set_scan_desc(&mut self, desc: pg_sys::TableScanDesc, rel: pg_sys::Relation) {
        self.scan_desc = desc;

        if !rel.is_null() {
            // SAFETY: rel is a valid, open Relation for the direct scan.
            let tupdesc = unsafe { (*rel).rd_att };
            if !tupdesc.is_null() {
                if self.direct_scan_slot.is_null() {
                    // SAFETY: tupdesc belongs to the opened relation and
                    // remains valid until the executor closes the scan.
                    self.direct_scan_slot = unsafe {
                        pg_sys::MakeSingleTupleTableSlot(
                            tupdesc,
                            &raw const pg_sys::TTSOpsBufferHeapTuple,
                        )
                    };
                }
                if self.direct_minimal_slot.is_null() {
                    // SAFETY: same TupleDesc as the buffered MinimalTuples.
                    self.direct_minimal_slot = unsafe {
                        pg_sys::MakeSingleTupleTableSlot(
                            tupdesc,
    ...
    /// Returns the direct table-scan slot when the executor owns one.
    #[must_use]
    pub(super) fn direct_scan_slot(&self) -> *mut pg_sys::TupleTableSlot {
        self.direct_scan_slot
    }

    /// Returns the direct MinimalTuple fallback slot when present.
    #[must_use]
    pub(super) fn direct_minimal_slot(&self) -> *mut pg_sys::TupleTableSlot {
        self.direct_minimal_slot
    }

    /// Returns a relation-shaped slot suitable for MinimalTuple extraction.
    #[must_use]
    pub(super) fn expression_extract_slot(
        &self,
...
impl Drop for ScanExecState {
    fn drop(&mut self) {
        if !self.direct_scan_slot.is_null() {
            // SAFETY: slot was created by MakeSingleTupleTableSlot.
            unsafe {
                pg_sys::ExecDropSingleTupleTableSlot(self.direct_scan_slot);
            }
            self.direct_scan_slot = std::ptr::null_mut();
        }
        if !self.direct_minimal_slot.is_null() {
            // SAFETY: slot was created by MakeSingleTupleTableSlot.
            unsafe {
                pg_sys::ExecDropSingleTupleTableSlot(self.direct_minimal_slot);
            }
            self.direct_minimal_slot = std::ptr::null_mut();
        }
