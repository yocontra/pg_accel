...
/// Drop every compatibility cache while the GPU runtime is still alive.
pub(crate) fn cleanup_backend_caches() {
    SSBM_Q1_CACHE.with(|slot| *slot.borrow_mut() = None);
    RESIDENT_DENSE_GROUP_AGG_CACHE.with(|slot| *slot.borrow_mut() = None);
    RESIDENT_STAR_DIM_GROUP_AGG_CACHE.with(|slot| *slot.borrow_mut() = None);
    RESIDENT_H3_GROUP_AGG_CACHE.with(|slot| *slot.borrow_mut() = None);
    RESIDENT_HASH_JOIN_COUNT_CACHE.with(|slot| *slot.borrow_mut() = None);
}
