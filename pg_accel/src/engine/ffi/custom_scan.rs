        ...
        (*state).accel.executor = if privdata.gpu_strategy == GpuStrategy::PreAgg {
            ...
            if privdata.self_scan_relid > 0 {
                ...
                if !rt_entry.is_null() {
                    ...
                    let vscan = VectorizedScan::new(sd, key_attno, key_typid);
...
struct CustomPrivateData {
    ...
    /// Relation index for VectorizedScan (Sort with direct heap scan).
    /// 0 means no vectorized scan (use child plan instead).
    self_scan_relid: u32,
}
