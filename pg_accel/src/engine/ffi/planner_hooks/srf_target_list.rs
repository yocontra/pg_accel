...
//! list. PostgreSQL wraps each such path in a [`pg_sys::ProjectSetPath`]
...
unsafe fn wrap_projectset_path(
    ...
    unsafe {
        ...
        if !super::add_gpu_path_with_resident_proof(
            "srf_target_list_no_resident_pipeline",
            output_rel,
            cpath,
            custom_scan::resident_proof_default_for_strategy(
                custom_scan::GpuStrategy::SrfTargetList,
            ),
        ) {
            return false;
        }
...
///    (one input row -> many output rows) under PostgreSQL's native
///    `ProjectSet` while selected CustomScans are hard-gated on a
///    GPU-resident pipeline.
/// 2. The EXPLAIN plan stays native and records `no_gpu_resident_pipeline`.
...
mod tests {
    ...
    fn upper_relation_kind_final_is_seven() {
        // Sanity: PostgreSQL currently numerically encodes UPPERREL_FINAL as 7. If the
    ...
    fn pg_test_srf_tlist_h3_grid_disk_expands() {
        ...
        assert_eq!(
            ...
            "h3_grid_disk(cell, 1) over 2 non-pentagon cells must expand to 14 rows \
             (2 * 7); got {count}. Native PG ProjectSet must satisfy this \
             row-count contract while selected SRF CustomScans are resident-gated.",
        );
    }

    /// `EXPLAIN SELECT id, h3_grid_disk(cell, 1) FROM h3_cells` plan
    /// must stay native until SRF expansion can remain GPU-resident.
    #[pg_test]
    fn pg_test_srf_tlist_explain_stays_native_until_resident() {
        if !ensure_extension("h3") {
            return;
        }
        Spi::run("SELECT h3_get_resolution('8928308280fffff'::h3index)").expect("h3 ping");
        ...
        assert!(
            ...
            "Expected SRF target-list plan to stay native until GPU-resident; got:\n{plan}",
        );
        assert!(
            plan.contains("ProjectSet"),
            "Expected native ProjectSet while SRF target-list CustomScan is resident-gated; \
             got:\n{plan}",
        );
        let rejection = Spi::get_one::<String>("SELECT pg_accel_last_planner_rejection_reason()")
            .expect("last rejection query should succeed")
            .expect("resident-only SRF decline should record a reason");
        assert_eq!(
            rejection, "no_gpu_resident_pipeline",
            "SRF target-list should expose the hard resident-only decline; plan:\n{plan}",
        );
    }
    ...
    fn pg_test_srf_tlist_large_varlen_declines_custom_scan() {
        ...
        assert_eq!(
            rejection, "no_gpu_resident_pipeline",
            "large target-list SRF should expose the hard resident-only decline before \
             SRF-specific admission checks; plan:\n{plan}"
        );
    }
    ...
    fn pg_test_srf_tlist_multi_srf_records_semantics_decline() {
        ...
        assert_eq!(
            rejection, "no_gpu_resident_pipeline",
            "multi-SRF target list should expose the hard resident-only decline before \
             SRF-specific admission checks; plan:\n{plan}"
        );
    }
