...
pg_accel does **not** use rayon. The accel arm's parallelism comes from
synchronous GPU dispatch plus any PostgreSQL parallel worker processes in native
plans; current executors do not spawn CPU worker threads. `pg_accel.max_workers_total`
is a superuser-settable budget for future pg_accel-owned host worker thread
requests, defaulting to `0` for unlimited, but it is not an auto-derived worker
count. Parallel workers in the baseline arm are normal PG parallel query
(separate backend processes, not threads).
