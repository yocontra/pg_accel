...
| **FP64** | native | native | varies by GPU | soft-fp64 via pinned `yocontra/soft-fp` (`ACPP_SOFT_FP64_SRC_DIR`; ~1/32×–1/64× fp32 perf) | native |
...
1. **fp64 via soft-double at 1/32–1/64× fp32 perf.** Apple GPUs do not expose native fp64, but pg_accel treats fp64 as available on Metal through the pinned `yocontra/soft-fp` library consumed by AdaptiveCpp via `ACPP_SOFT_FP64_SRC_DIR`. `has_native_fp64` is a cost-model signal only: it must never be used as a dispatch gate. Missing soft-fp64 support is an installation/toolchain failure to fix, not a reason to select a CPU-backed pg_accel plan or silently disable fp64 SQL shapes.
...
ACPP_BACKEND=metal just setup-gpu
```
...
pgaccel_caps detect(sycl::device& d) {
    pgaccel_caps c{};
    // Hardware fp64 only. Metal soft-fp64 still dispatches when this is false.
    c.has_native_fp64 = d.has(sycl::aspect::fp64);
...
- **fp64 via soft-double** — MSL emitter lowers `double` to `struct acpp_f64 { uint lo; uint hi; }` with per-op dispatch to `__acpp_sscp_soft_f64_*` symbols. Math bodies come from the pinned `yocontra/soft-fp` repo (`v1.3.0` per `TODO.md`) and AdaptiveCpp is built with `ACPP_SOFT_FP64_SRC_DIR` pointing at that source. Metal's `sycl::aspect::fp64` remains a hardware/native capability probe, so pg_accel records it as `has_native_fp64` for costing only. The correct policy is: dispatch fp64 kernels on Metal, charge the soft-fp64 multiplier in the planner, and fix any kernel-family gap that still declines solely because the path is fp64. Current root fixes in the fork-safe Metal path include SLEEF helper address-space specialization, unreachable soft-fp64 function/global pruning, demand-driven retention of emitter-implicit fp64 primitives, MSL `__attribute__((noinline))` emission for LLVM `noinline` helpers, null/undef pointer PHI handling, and suppression of generated selector string constants that were only used by Metal inline-call lowering. Local evidence from 2026-07-04 after the upstream sync: fp64 probes for add/mul/sqrt/sin/cos/asin/atan2/haversine pass with zero mismatches and no unused-selector MSL warnings, `test_spatial` passes `162/162`, `test_h3` passes `856/0`, `test_correctness` passes `340/340`, and fork/cold-fork/warmed-fork plus the 8x20 archive fork stress pass.
...
- **llvm.minnum/maxnum** — IEEE 754-2008 NaN + signed-zero semantics are preserved in two layers: (1) the Metal SSCP libkernel body for `__acpp_sscp_fmin_f32` / `__acpp_sscp_fmax_f32` in `src/libkernel/sscp/metal/math.cpp` implements the NaN-propagating + `(−0, +0)` signed-zero-preserving sequence directly (always-inlined before MetalEmitter runs); (2) `Emitter.cpp` emits a defensive NaN-only fallback for any call that survives inlining. `-ffast-math` is therefore safe and **enabled globally** in `pgaccel-kernels/CMakeLists.txt`, with per-file `-fno-fast-math` opt-outs for the kernels that depend on NaN/Inf propagation:
  - `raster_ops.cpp` — resident metadata and NODATA validation use `isfinite`.
  - `spatial_predicates.cpp` — `isfinite` gates NaN/Inf coordinates to UNCERTAIN.
  - `spatial_dispatch.cpp` — resident fp64 validation and uncertainty gates depend on finite intermediates and signed-zero semantics.
  - `bbox_ops_f64.cpp` — raw IEEE-754 ordering preserves conservative NaN/Inf behavior without invoking Metal fp64 lowering.
  - `reduce.cpp` — fused multi-reduce NaN propagation and `±infinity` MIN/MAX identities are part of the PostgreSQL-compatible contract.
  - `expr_eval.cpp` — PostgreSQL-compatible comparisons detect NaN with `x != x`, which finite-math optimization may fold away.
  - `grouped_agg.cpp` — floating predicates and MIN/MAX use explicit NaN detection and PostgreSQL NaN ordering.
