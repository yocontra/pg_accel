...
BEGIN
    FOR r IN EXPLAIN
        SELECT id FROM _rg_tiny_geo a, _rg_tiny_geo b
...
BEGIN
    FOR r IN EXPLAIN (FORMAT JSON)
        SELECT id FROM _rg_json_pts a, _rg_json_pts b
        WHERE ST_DWithin(a.geom::geography, b.geom::geography, 100)
...
-- After toggling, do a spatial query to make sure state is clean
SET pg_accel.enabled = on;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _rg_toggle_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '89_regression: after toggle, pg_accel not in query plan';
    END IF;
END $$;

-- =========================================================================
-- 15-16. Large spatial table -- verify Custom Scan IS present
-- =========================================================================
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _rg_plan_large WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '89_regression: large spatial table: pg_accel not in query plan';
    END IF;
END $$;
