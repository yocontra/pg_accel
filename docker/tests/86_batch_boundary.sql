-- 86_batch_boundary.sql: Batch boundary behavior with spatial queries
...
-- ========== Test 3: 4096-row EXPLAIN shows Custom Scan + ON/OFF compare ==========
SET pg_accel.enabled = on;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _bb_plan_4096 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '86_batch: 4096-row table: pg_accel not in query plan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _bb_plan_4097 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '86_batch: 4097-row table: pg_accel not in query plan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _bb_plan_8192 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '86_batch: 8192-row table: pg_accel not in query plan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _bb_grp_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '86_batch: GROUP BY spanning batches: pg_accel not in plan';
    END IF;
END $$;
