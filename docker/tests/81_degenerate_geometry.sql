-- 81_degenerate_geometry.sql: Degenerate geometry handling for GpuSpatial functions.
...
INSERT INTO _dg_geoms (label, geom) VALUES
    -- Empty geometries
    ('point_empty',        'POINT EMPTY'::geometry),
    ('line_empty',         'LINESTRING EMPTY'::geometry),
    ('poly_empty',         'POLYGON EMPTY'::geometry),
    ('gc_empty',           'GEOMETRYCOLLECTION EMPTY'::geometry),
...
-- Bulk data for ensuring enough rows for Custom Scan threshold
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _dg_plan1 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '81_degen FAILED: st_intersects not using Custom Scan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _dg_plan2 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '81_degen FAILED: st_contains not using Custom Scan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _dg_plan3 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '81_degen FAILED: st_within not using Custom Scan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _dg_plan4 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '81_degen FAILED: st_dwithin not using Custom Scan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _dg_plan10 WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '81_degen FAILED: high-vertex st_contains not using Custom Scan';
    END IF;
END $$;
