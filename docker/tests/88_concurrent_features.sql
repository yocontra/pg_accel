-- 88_concurrent_features.sql: pg_accel with PostgreSQL features
-- Tests parallel workers, multi-predicate WHERE, VIEWs, materialized views,
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _cf_plan_par WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '88_concurrent: parallel + spatial: pg_accel not in query plan';
    END IF;
END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _cf_plan_multi WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '88_concurrent: multi-predicate: pg_accel not in query plan';
    END IF;
END $$;
...
-- =========================================================================
-- 7. Materialized view with spatial source
-- =========================================================================
SET pg_accel.enabled = on;
CREATE MATERIALIZED VIEW _cf_matview AS
SELECT p.id, p.cat
FROM _cf_points p, _cf_ref r
WHERE ST_DWithin(p.geom::geography, r.geom::geography, 800);
...
DO $$ BEGIN
    IF (SELECT count(*) FROM _cf_matview) IS DISTINCT FROM
       (SELECT count(*) FROM _cf_matview_off) THEN
        RAISE EXCEPTION '88_concurrent: matview row count mismatch';
    END IF;
END $$;
...
DROP MATERIALIZED VIEW IF EXISTS _cf_matview;
