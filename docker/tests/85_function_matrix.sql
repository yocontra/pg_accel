-- 85_function_matrix.sql: Every GPU-accelerated function with diverse inputs
-- Tests GpuSpatial, GpuH3, GpuRaster function groups on 5000+ row tables.
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_si_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_si_plan FAILED: pg_accel not in plan for ST_Intersects';
    END IF;
END $$;

\echo 'PASS: 85_si_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_sc_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_sc_plan FAILED: pg_accel not in plan for ST_Contains';
    END IF;
END $$;

\echo 'PASS: 85_sc_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_sw_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_sw_plan FAILED: pg_accel not in plan for ST_Within';
    END IF;
END $$;

\echo 'PASS: 85_sw_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_dw_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_dw_plan FAILED: pg_accel not in plan for ST_DWithin';
    END IF;
END $$;

\echo 'PASS: 85_dw_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_h3c_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_h3c_plan FAILED: pg_accel not in plan for h3_latlng_to_cell';
    END IF;
END $$;

\echo 'PASS: 85_h3c_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_gr_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_gr_plan FAILED: pg_accel not in plan for h3_get_resolution';
    END IF;
END $$;

\echo 'PASS: 85_gr_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_cp_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_cp_plan FAILED: pg_accel not in plan for h3_cell_to_parent';
    END IF;
END $$;

\echo 'PASS: 85_cp_plan'
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM _fm_gd_plan WHERE line ILIKE '%custom scan%') THEN
        RAISE EXCEPTION '85_gd_plan FAILED: pg_accel not in plan for h3_grid_distance';
    END IF;
END $$;

\echo 'PASS: 85_gd_plan'
...
BEGIN
    ...
    IF has_raster THEN
        ...
        BEGIN
            ...
            EXECUTE '
                DO $inner$ BEGIN
                    IF (SELECT count(*) FROM _fm_r29_on) IS DISTINCT FROM
                       (SELECT count(*) FROM _fm_r29_off) THEN
                        RAISE EXCEPTION ''85_r29 FAILED: ST_Clip row counts differ ON vs OFF'';
                    END IF;
                END $inner$';
        ...
        BEGIN
            ...
            EXECUTE '
                DO $inner$ BEGIN
                    IF (SELECT count(*) FROM _fm_r30_on) IS DISTINCT FROM
                       (SELECT count(*) FROM _fm_r30_off) THEN
                        RAISE EXCEPTION ''85_r30 FAILED: ST_MapAlgebra counts differ ON vs OFF'';
                    END IF;
                END $inner$';
