...
|---|---|---|---|
| Resident reducing or grouped aggregate | Present | Selectable | Covered `AggQuerySpec` shapes become `Custom Scan (GpuAccelAgg)` after shape, type, residency, device, and cost gates. This includes qualified 250K–1M nullable bool-key / distinct nullable bool, int2, int8, date, timestamp, or timestamptz `COUNT(column)` envelopes; global, filtered, joined, non-boolean-grouped, `HAVING`, and broader count variants remain native. Stored generated columns are physical base attributes and remain selectable when their types and aggregate shape pass those same gates. |
