...
source scripts/pg_versions.sh && pg="$(pg_accel_default_pg_major)" && port="$(pg_accel_pgrx_port_for_pg "$pg")" && cargo run -p pg_accel_bench -- fp64-calibrate --connection "host=localhost port=${port} dbname=postgres" # soft-fp64 multiplier sweep; omit --max-size for release evidence
```
...
| `pg_accel.kernel_timeout_ms` | `5000` | Warning threshold for elapsed synchronous GPU kernel calls; not async cancellation |
| `pg_accel.max_workers_total` | `0` | Cluster-wide cap for pg_accel-owned host worker thread requests; 0 = unlimited |
| `pg_accel.cost_multiplier` | `1.0` | Cost estimate multiplier (>1 = more conservative) |
| `pg_accel.assert_dispatch` | `false` | Benchmark guard that warns when a large-enough query is planner-declined |
| `pg_accel.parallel_fused_count` | `false` | Superuser roadmap knob; current PG18 shape remains native with `parallel_fused_count_unstable` |
| `pg_accel.otel_log_max_mb` | `256` | Per-file trace JSONL cap in MiB |
| `pg_accel.otel_log_max_rotations` | `4` | Retained rotated trace JSONL copies |
| `pg_accel.fp64_enabled` | `true` | Kill switch for fp64 GPU dispatch |
| `pg_accel.soft_fp64_cost_multiplier` | `32.0` | Planner cost multiplier for fp64 work on non-native-fp64 devices |
