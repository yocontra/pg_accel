...
- [ ] Benchmark ratchets fail CI when a selected GPU cell regresses below PostgreSQL parallel parity, crashes, silently misses GPU dispatch, or loses expected GPU plan selection. The checked-in qualified-Metal CI and release jobs now run the immutable six-cell 1M-row `metal-ship-gate` contract in release mode with seed 42, raw timing, cache mode `both`, ten measurements, five warmups, complete plan/dispatch/correctness artifacts, 1.00x general floors, the 1.50x H3 bulk floor, and the 1.10x fused H3 parent floor. The command rejects incomplete or altered evidence, and release creation depends on the gate job. Workflow wiring is not execution evidence; a successful exact-candidate CI artifact remains required. Evidence SHA/CI: `<sha-or-url>`.
...
- [ ] **External/publication:** Coverage reaches at least 90% for pg_accel-owned Rust, C++/SYCL, and SQL-extension behavior, and CI publishes the exact-candidate bundle while failing below any required threshold. The local precursor seals independent Rust source-line, C++/SYCL source-line (host instrumentation plus real Metal device counters), and fixed-manifest SQL semantic-assertion layers at 90% each; it cannot close this publication gate. The sealed SQL scope contains 52 files and 287 assertions. Evidence CI/artifact: `<sha-or-url>`.
